...
This repository currently contains **no source code, no build system, no tests, and no package manifest**. It holds two authoritative source documents plus eight documents derived from them, for a system that has not been built:
...
Eight such documents now exist, generated in this order, each from its own prompt in `docs/prompts/` and each ending in its own `## <Name> Readiness` verdict (`READY FOR NEXT DOCUMENTATION PHASE`) that gates the next one:
...
The next document in the chain is `eval.md`, followed by `SCALING.md`, `implementation-plan.md`, ADRs — none of these exist yet; generate and review them one at a time, in that order, and do not skip ahead. Every one of these downstream documents treats the eight already-generated ones above as authoritative-alongside the original seven — subordinate to them if either conflicts, per each document's own `## SOURCE CONTRADICTIONS` section.
