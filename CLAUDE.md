...
These six documents are the stabilized baseline the rest of the documentation set builds on. `docs/scenario-matrix.md` is the next document in the chain: a populated scenario cross-validation matrix (currently 263 scenarios across 30 domains) generated from `docs/prompts/generate-scenario-matrix.prompt.md` and reconciled against the baseline above — it has passed its own readiness check (see `## 47. Scenario Deduplication and Completeness Review` at the end of the file, verdict: READY FOR DOCUMENTATION BASELINE FREEZE) and must pass it again before any later optimization-specific document — `optimization-catalog.md`, `provider-matrix.md`, `cache-strategy.md`, `agent-optimization.md`, `inference-optimization.md`, `quality-gates.md`, `security.md`, `observability.md`, `eval.md`, `SCALING.md`, `implementation-plan.md`, ADRs — is generated. None of those later documents exist yet; generate and review them one at a time, in that order. The prompt for the first one, `docs/prompts/generate-optimization-catalog.prompt.md`, already exists in the repo — `optimization-catalog.md` itself does not yet.
...
- Every requirement in these docs is traceable via IDs: `OBJ-NNN` (35 objectives — OBJ-001–014 original, OBJ-015–022 from the first hardening amendment, OBJ-023–035 from the second), `SEC-NNN` (16 security requirements — SEC-001–010 original, SEC-011–016 from the second hardening amendment), `NFR-NNN` (14 non-functional requirements), `AC-NNN` (53 acceptance criteria), `EC-NNN` (213 edge cases — EC-001–079 original, EC-080–140 from the first hardening amendment, EC-141–213 from the second), `DA-NNN` (25 developer-agent modules), `OI/CL/CE/TE/AL/AR/QO/EL-NNN` (intelligence/lifecycle/economics modules), `SCN-<CODE>-NNN` (scenario matrix, 30 domain codes A–AD, currently 263 scenarios). When adding new material, give it an ID in the matching series and cross-reference it the way existing entries do, rather than inventing a new numbering scheme.
