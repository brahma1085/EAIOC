...
This repository currently contains **no source code, no build system, no tests, and no package manifest**. It holds two authoritative source documents plus six documents derived from them, for a system that has not been built:
...
Six such documents now exist, generated in this order, each from its own prompt in `docs/prompts/` and each ending in its own `## <Name> Readiness` verdict (`READY FOR NEXT DOCUMENTATION PHASE`) that gates the next one:
...
The next document in the chain is `security.md`, followed by `observability.md`, `eval.md`, `SCALING.md`, `implementation-plan.md`, ADRs — none of these exist yet; generate and review them one at a time, in that order, and do not skip ahead. Every one of these downstream documents treats the six already-generated ones above as authoritative-alongside the original seven — subordinate to them if either conflicts, per each document's own `## SOURCE CONTRADICTIONS` section.
...
- Every requirement in these docs is traceable via IDs: `OBJ-NNN` (35 objectives — OBJ-001–014 original, OBJ-015–022 from the first hardening amendment, OBJ-023–035 from the second), `SEC-NNN` (16 security requirements — SEC-001–010 original, SEC-011–016 from the second hardening amendment), `NFR-NNN` (14 non-functional requirements), `AC-NNN` (53 acceptance criteria), `EC-NNN` (213 edge cases — EC-001–079 original, EC-080–140 from the first hardening amendment, EC-141–213 from the second), `DA-NNN` (25 developer-agent modules — schema/applicability in `optimization-catalog.md`, full algorithms for the agent-loop-facing subset in `agent-optimization.md`), `OI/CL/CE/TE/AL/AR/QO/EL-NNN` (intelligence/lifecycle/economics modules — `AL-NNN`/`AR-NNN` fully elaborated in `agent-optimization.md`), `SCN-<CODE>-NNN` (scenario matrix, 30 domain codes A–AD, 263 scenarios). The six generated downstream documents add their own series on top of these: `TECH-NNN` (20 canonical optimization techniques) and `P5 Index` entries (11, index-level only) in `optimization-catalog.md`; `PROV-NNN` in `provider-matrix.md`; `CACHE-NNN` (exactly 7, one per canonical cache type) in `cache-strategy.md`; `INFOPT-NNN` (exactly 11, one per P5 capability, detection/routing/negotiation/measurement only) in `inference-optimization.md`; `QG-NNN` (exactly 10, one per `architecture.md` §28 quality dimension) in `quality-gates.md`. When adding new material, give it an ID in the matching series and cross-reference it the way existing entries do, rather than inventing a new numbering scheme.
