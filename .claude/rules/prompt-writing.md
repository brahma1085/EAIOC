...
Source: Anthropic "Prompting best practices" (platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices — the old standalone pages for individual techniques now redirect here), current Claude models per that page (Claude Fable 5.1, Claude Mythos 5.1, Claude Fable 5, Claude Mythos 5, Claude Opus 5, Claude Sonnet 5, Claude Haiku 4.5, and their immediate predecessors: Claude Opus 4.6-4.8, Claude Sonnet 4.6). Model names ship fast; re-verify this list against the live doc occasionally rather than trusting it indefinitely. These are not optional extras. They map directly to rubric dimensions.

1. **Be clear and direct.** Write for "a brilliant new employee who lacks context on your norms." State the exact output, format, and constraints. If you want above-and-baseline effort, say so explicitly ("go beyond the basics", "include as many relevant features as possible"); the model will not infer it from a vague ask. Golden-rule test: show the prompt to a colleague with no context on the task and have them follow it literally — if they'd be confused, the executor will be too.
...
7. **Say what to do, not what not to do.** "Write in flowing prose paragraphs" beats "do not use markdown." Pair a positive instruction with an XML format indicator when format matters (e.g. "write the prose sections in `<answer>` tags"). If steerability issues persist, match the prompt's own formatting to the desired output — a markdown-heavy prompt nudges a markdown-heavy answer, so strip markdown from the prompt itself to reduce markdown in the response.
...
10. **Steer thinking by goal, not by prescribed steps.** "Think thoroughly, then verify your answer against [criteria] before finishing" beats a hand-written step list; the model's own reasoning often exceeds a prescribed plan. When thinking is off, ask it to reason in `<thinking>` tags and answer in `<answer>` tags — except on Opus 5, where disabling thinking can leak internal XML into the visible output (and is only possible at effort `high` or lower to begin with), so prefer keeping thinking on at a lower effort level there instead of this pattern. Note: with thinking off, Claude Opus 4.5 specifically is sensitive to the word "think"; prefer "consider", "evaluate", "reason through". Do not bolt on a generic "verify your work before finishing" clause when the target executor is Opus 5 — it self-verifies well by default, and an added instruction causes over-verification (extra tokens and latency) rather than better answers; keep that instruction for other models.
...
13. **Do not use prefill.** Since Claude 4.6, a prefilled last-assistant-turn returns a hard 400 error (earlier models still accept it, but this rule targets a fresh session on a current model). For structured output, instruct the schema directly (or name Structured Outputs / tool enums); for preambles, instruct "respond directly, no 'Here is...' lead-in"; do not fake a prefix.
...
15. **Forbid shortcut and hardcoded solutions on code tasks.** Add "implement a solution that works for all valid inputs, not just the given test cases; do not hardcode values or special-case test inputs; if a test itself looks wrong or the task is infeasible, say so rather than working around it." Tests verify correctness — they don't define the solution — and this is distinct from the overengineering guard in item 12.
...
- [ ] Self-verification step: how the executor proves the output is correct before handing back. Soften or omit this when the named executor is Opus 5 — it self-verifies by default, and an explicit "check your work" clause causes over-verification instead of a better answer.
...
- **Code generation / refactor:** "Outputs" specifies files touched, function signatures, tests added. "Verification" mandates run-the-tests, lint, type-check, manual diff. Add "investigate before claiming", the overengineering constraint, and the anti-hardcoding constraint (general-purpose solution, not test-specific). "Anti-hallucination" forbids invented APIs, library names, file paths. If the executor may create scratch files or scripts to iterate, add "delete any temporary files or scripts you created for iteration once the task is done."
...
- **Automation / scripting / agent orchestration:** "Inputs" includes env, secrets handling, dry-run mode. "Verification" mandates dry-run output reviewed before live run. Add subagent-scope guidance ("use subagents only for parallel/isolated workstreams; work directly for simple, sequential, single-file tasks"). "Failure modes" includes idempotency, rate limits, partial state. If the executor can take destructive or hard-to-reverse actions (deleting files, force-push, posting to external services), add an explicit confirm-before-acting constraint naming those action classes.
...
- Code prompt with tests but no "solve the general case, don't hardcode to the tests" constraint: -3
